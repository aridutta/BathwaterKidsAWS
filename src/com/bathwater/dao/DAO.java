...
public class DAO implements DAOInt{
    ...
    private static IDynamoDBHelper helper;
	private static IDynamoDBScans scanHelper;
	private static IDynamoDBQueries queryHelper;
    private static IESItemQueries esQueryHelper;
    private static Random random;
	
	public DAO(IDynamoDBHelper helper, IDynamoDBScans scanHelper, IDynamoDBQueries queryHelper, IESItemQueries esQueryHelper, Random random){
		this.helper = helper;
		this.scanHelper = scanHelper;
		this.queryHelper = queryHelper;
		this.esQueryHelper = esQueryHelper;
		this.random = random;
	}
	
    public synchronized static DAO getInstance() {
    	if(INSTANCE == null){
    		INSTANCE = new DAO(helper, scanHelper, queryHelper, esQueryHelper, random);
    	}
        return INSTANCE;
    }
...
//            String token = StringUtil.generateRandomNumbers(5);
            String token = StringUtil.generateToken(params.getEmail());
            user.setToken(token);
...
//              String token = StringUtil.generateRandomNumbers(5);
	            String token = StringUtil.generateToken(user.getEmail());
	            user.setToken(token);
            }else {
...
//            String token = StringUtil.generateRandomString(5);
            String token = StringUtil.generateToken(user.getEmailAddress());
            ...
            if (userList != null && !userList.isEmpty()) {
                ...
                if ((userList.get(0).getPassword().equals(password) && userList.get(0).getActive() == 1)
                    ...
                    if (user.getToken() == null) {
                        ...
                        String token = StringUtil.generateToken(userList.get(0).getEmailAddress());
                    ...
                    try{
                    	Claims claimsB = StringUtil.getIdFromToken(user.getToken());
                        String userId = (String) claimsB.get("userId");
                        if(!userList.get(0).getEmailAddress().equals(userId)){
                        	res.setErrorCode(205);
                            res.setMessage("invalid token");
                        }
                    }
                    	...
                    	res.setErrorCode(205);
                        res.setMessage("coding format not found");
                        return res;
                    }
                    catch(Exception e){
                    	res.setErrorCode(205);
...
//        res.setMessage("invalid item");
    ...
    public ErrorResult updateCredits(String sid,int credits) {
        ...
        item.setSharable(item.getSharable());
    ...
    public ErrorResult borrowItem(BorrowItemParameters params) {
        ...
        if (storedItems.get(0).getSharable() != 1) {
...
//                String token = StringUtil.generateRandomString(5);
            	String token=null;
				try {
					token = StringUtil.generateToken(params.getEmail());
				} catch (UnsupportedEncodingException e) {
...
//        }
        ...
        if (pickedUpBins != null && !pickedUpBins.isEmpty()) {
            if (bins == null) {
                ...
                for (PickupList.Bin pickedupBin : pickedUpBins) {
                    InventoryTableItem bin = ((DynamoDBScans) scanHelper).getBinByBinNumber(pickedupBin.getBinCode()).get(0);
                    bin.setImageURLs(pickedupBin.getImageBase64());
                    helper.putItem(bin);
                }
            }
